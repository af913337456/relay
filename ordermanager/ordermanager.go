...
// tokenAddress: order.RawOrder.TokenS
// value:是要卖的币的余额 TokenSBalance，不是客户端传过来的
func (om *OrderManagerImpl) IsValueDusted(
	tokenAddress common.Address, value *big.Rat) bool {
	// lgh: 获取数量乘上汇率后的真实的价格，单位基于 currencyStr
	if legalValue, err :=
		om.mc.LegalCurrencyValue(tokenAddress, value); nil != err {
		return false
	} else {
